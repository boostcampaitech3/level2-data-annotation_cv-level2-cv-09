...
class SceneTextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for word_info in self.anno['images'][image_fname]['words'].values():
            vertex = np.array(word_info['points']).flatten()
            # print(vertex)

            if len(vertex) > 8:
                bbox = np.array(word_info['points'])
                # print(bbox)
                rect = [bbox[:, 0].min(), bbox[:, 1].min(), bbox[:, 0].max(), bbox[:, 1].min(), bbox[:, 0].max(), bbox[:, 1].max(), bbox[:, 0].min(), bbox[:, 1].max()]
                vertex = np.array(rect).flatten()
                # print(vertex)
